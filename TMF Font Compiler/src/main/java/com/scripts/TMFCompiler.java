...
public class TMFCompiler {
	...
	private BufferedImage fontImg; // the font image

	private File outputFile; // the output file
	...
	private Map<String, Object> fontInfos; // the content of the font file infos
	...
	public TMFCompiler(String[] args) {
		...
		fontImg = new FontImgReader(fontImgFile.getAbsolutePath()).getImage(); // the font image
		fontInfos = new FontInfoReader(fontInfosFile.getAbsolutePath()).getContent(); // the font infos content
	...
	private void compile() {
		...
		List<Object> chars = new ObjectMapper().convertValue(fontInfos.get("chars"), new TypeReference<List<Object>>() {
			...
			for (int i = 0; i < pixels.size(); i++) {
				...
				if (curPixel[0] == 0 && lineStart != -1 && lineColorType != LineColors.TRANSPARENT) {
					...
					if (lineColorType == LineColors.TRANSPARENT) {
						...
						warnings.add("the line at position " +
								x + " does not start with black, because white was found.");
					}
		...
		System.out.print(" done\n\n\ncompilation finished " + (warnings.size() == 0 ? "successfully " : "") +
				"in " + (System.currentTimeMillis() - startTime) / 1000.0f + "s with " + warnings.size() + " warnings");
