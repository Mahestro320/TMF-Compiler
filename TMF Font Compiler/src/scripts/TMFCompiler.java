...
public class TMFCompiler {
	...
	public TMFCompiler(String[] args) {
        ...
        // if the number of args is invalid (less than 2 or more than 3), show the message to explain how to
		// use the launch compiler command
        if (args.length < 2 || args.length > 3) {
            System.out.println("usage: java Main.java <fontImageInputPath> <fontInfoInputPath> <fontData" +
							   "OutputPath (optional, default is src\\out)>\n\nthe paths is relative to " + 
							   "src folder\n\nfor example: \"java Main.java input\\font.png input\\font." + 
							   "txt\"");
            return;
        }
        ...
        if (!fontImgFile.exists() && !fontInfosFile.exists()) {
            invalidFile = fontImgFile.getAbsolutePath() + " and " + fontInfosFile.getAbsolutePath();
        }
	...
	private void compile() {
		...
		// encode font name
		byte[] fontNameBytes = fontInfos[0].getBytes(StandardCharsets.UTF_8);
		...
		for (int i = 1; i < 256; i++) {
			...
			if (curChar.length() == 0) {
				...
				try {
					...
					System.out.println("\nError: the char " + String.valueOf(curChar.charAt(0)) +
									   " is not an valid number.");
					return;
				}
				...
				if (curChar.length() == 1) {
					...
					addBinVal(charsLength, Integer.toHexString(
						Integer.parseInt(String.valueOf(curChar.charAt(0))) + 6));
		...
		// a string[] of two hexadecimal values, for example {"E", "3"} -> \xE3 for line positions
		...
		// if no opaque pixels found in the image, show an warning and supress warnings of the
		...
		System.out.print("in " + Math.round((System.currentTimeMillis() - startTime) / 1000) + "s ");
	...
	private void showFinishText(int warningsNumber) {
		if (warningsNumber != 0) {
			System.out.print(" -> " + warningsNumber + " warning");
