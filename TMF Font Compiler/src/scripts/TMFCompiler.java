...
public class TMFCompiler extends CompilerTools {
	...
	*/
	...
	public TMFCompiler(String[] args) {
        ...
        } else if (!fontImgFile.exists()) {
            invalidFile = fontImgFile.getAbsolutePath();
        
        } else if (!fontFileInfosFile.exists()) {
            invalidFile = fontFileInfosFile.getAbsolutePath();
        }

        if (invalidFile != null) {
            System.out.println("Error: the file \"" + invalidFile + "\" does not exists.");
            return;
        }
